...
::: warning 空の結果は「要件が無い」という意味ではありません
...
::: danger `true` を準拠の証明として扱わないでください
...
::: details 受動態で書かれた禁止（`MUST NOT be <過去分詞>`）
...
::: details 限定語の言い換え（`without` → `has no` / `lacks`）
