...
（765 / 950 / 917 / 1011 / 1042 / 741）。

### Fixed

- **番号を持たない見出しに、同じ番号を 2 度振っていた**:
  - 段の深さに上限があるため、上限より深い見出しが 2 つの塊に分かれて現れると
    同じ番号が 2 度出る。RFC 765 は転送形式の一覧（`ASCII Format` …）と
    ページ構造の一覧（`Header Length` …）がどちらも 4 段目に来て、
    **`3.1.1.1` から `3.1.1.5` が 2 度**立っていた（`A1` が見つけた）。
  - 番号が重なると `findSection` がどちらを引くか定まらない。既に使った番号は
    飛ばす。

- **文書名を二重引用符で書く別文書参照を見ていなかった**:
  - 1980〜90 年代の RFC は角括弧の目印を使わず、題名をそのまま引く。
    RFC 1191 §3.1 の
    `Section 4.2.2.6 of "Requirements for Internet Hosts -- Communication
    Layers"` は §4.2.2.6 をこの RFC の節として返していた（RFC 1191 に
    §4.2.2.6 は無い）。

- **`get_related_sections` が実在しない節を返していた**:
  - 原文が実在しない節を指していることがある。RFC 1323 §1.2 は
    `as explained in Section 5.3 and Appendix B` と書くが、RFC 1323 に §5.3 は
    無い。番号だけ返しても引けないので、題名を `Unknown` にして返していた。
  - 実在する節だけを返す。原文の側の食い違いは `D1`（相互参照の参照先が
    実在する）が引き続き測る。

### Added

- **corpus を 142 本 → 156 本にした**。足したのは 9449 / 9420（2020 年代 XML）、
  3492 / 4193（2000 年代）、1889（1990 年代後半）、1191 / 1323 / 1583
  （1990 年代前半）、1011 / 1042 / 950 / 917 / 765（1980 年代）、741（1970 年代）。
- **テストを 2 つ足した**（529 → 531）

### Known

- `D1` に RFC 1323 を、`E7` に RFC 741 を、`F3` に RFC 9449 を記録した。
  - RFC 1323 §1.2 の §5.3 は原文の側の食い違い。
  - RFC 741 の参考文献は RFC 733 / 822 と同じく目印を持たない段落が並ぶ。
  - RFC 9449 §4.3 の 1,435 文字は
    `To validate a DPoP proof, the receiving server MUST ensure the following:`
    に続く 12 項目を取り込んだもので、1 件の要件として正しい。

## [0.6.50] - 2026-09-04

**corpus を 127 本から 142 本に広げ、6 件直した。** RFC 822（1982 年のメール
書式）が 3 件、RFC 7515（JWS）が 1 件、ほかに 2 件。

### Fixed

- **中央に `- N -` と書くページフッタを落としていなかった**:
  - フッタは `[Page N]` の形だけを見ていた。RFC 822 は
...
- **文書全体が字下げされている RFC を読めていなかった**:
  - RFC 822 は本文も見出しも 5 桁目から組む。節見出しは 1 桁目から始まる前提で
    見ているため、**節が 1 つも取れていなかった**。取れていた 104 件はページの
    柱と本文の折り返しである。
  - 最小の字下げを測って、その分を外してから読む。実測（RFC 142 本）: 最小の
    字下げが 1 桁以上あるのは RFC 822 だけで、ほかはすべて 0 桁である。
  - 実測: RFC 822 の節が **104 → 121**。うち本物の節が 0 → 121。

- **題名を空行で割る RFC の題名を取れていなかった**:
  - RFC 822 と RFC 733 は
    `STANDARD FOR THE FORMAT OF` / 空行 / `ARPA INTERNET TEXT MESSAGES` と
    書く。最初の空行で切ると前半だけになる。
  - **前置詞・接続詞で終わっているときだけ**続きを取る。完結している題名の
    あとに続く行（著者名や日付）は取らない。

- **付録の題名が小文字の引用で始まると落としていた**:
  - 1 段目の付録は題名が大文字か数字で始まることを求めていた。本文の
    `B. Smith` のような行を拾わないための規則だが、`Appendix` の語がある
    ときにも課していた。RFC 7515 の
    `Appendix B.  "x5c" (X.509 Certificate Chain) Example` が落ち、
    **「A の次は B」の順番が合わなくなって続く C〜F も落ちていた**。
  - `Appendix` と書いていないときだけ課す。実測: RFC 7515 の節 76 → 81。

- **番号のあとに句点が入る別文書参照**:
  - RFC 8628 §3.1 は `as described in Section 3.2.1. of [RFC6749].` と書く。
    句点を認めていなかったため、RFC 8628 に無い §3.2.1 を返していた。
    突き合わせの `X10` / `X13` が見つけた。

### Added

- **corpus を 127 本 → 142 本にした**。足したのは 9369 / 9460（2020 年代 XML）、
  8415 / 8628 / 7515 / 7517 / 6298（2010 年代）、4585 / 5755 / 3412 / 4648
  （2000 年代）、1918 / 2046（1990 年代後半）、822 / 1057（1980 年代）。
- **テストを 4 つ足した**（525 → 529）

### Known

- RFC 822 を `A5`（題名）と `E7`（参照 0 件）に記録した。字下げを外したことで
  ヘッダ塊の形が変わり、本文からの題名が取れなくなった（`get_rfc_structure` は
  Datatracker の題名に落とすので、返る値は空にならない）。参考文献は RFC 733 と
  同じく目印を持たない段落が並ぶ形である。
- `A6` に RFC 2046 と RFC 5755 を、`D1` に RFC 4585 / 5755 / 9369 を記録した。
  RFC 9369 §3.3.2 は `The labels used in [QUIC-TLS] to derive packet protection
  keys (Section 5.1), header protection keys (Section 5.4), …` と書く。文書名は
  文頭に 1 度しか出ず、括弧の中の節番号とは離れている。RFC 6376 の `ibid` と
  同じ形で、機械では結び付けられない。

## [0.6.49] - 2026-09-04

**corpus を 112 本から 127 本に広げ、4 件直した。** RFC 4949（セキュリティ
用語集）と RFC 951（BOOTP）・RFC 3164（syslog）が、これまでに無い形を出した。

### Fixed

- **法令の条番号を、この RFC の節として拾っていた**:
  - RFC 4949 は米国法を `Section 111(d)`、`Section 552` (FOIA)、
    `Title 40 U.S.C. Section 1552` と引く。これらを §111 / §552 / §1552 と
    して返していた。
  - 最上位の番号は 2 桁までとする。実測（RFC 127 本）: 3 桁以上の節を持つ
    RFC は **1 本も無い**。番号の後ろに数字が続かないことも求める
    （求めないと `111` の頭 2 桁を採って §11 になる）。

- **前置詞が `in` の別文書参照を見ていなかった**:
  - RFC 5751 §2.5 は `- Message Digest (section (Section 11.2 in [CMS])` と
    書く。`of` の形だけを見ていたため、RFC 5751 に無い §11.2 を返していた。

- **番号にピリオドを置く参考文献の形**:
  - RFC 951 は `   1.  Ross Finlayson, …  RFC 903, NIC, June, 1984.` と書く。
    ピリオドを認めていなかったため、参考文献の欄はあるのに参照が 0 件だった。
    実測: RFC 951 で 9 件。
  - **字下げを求めるのが要点。** 求めずに直したところ、RFC 3629 の
    `14.  Informative References` `16.  Intellectual Property Statement` を
    項目として拾い、参照が 3 件増えて題名が番号のままになった（`E5` が
    見つけた）。参考文献の項目は必ず字下げされている。

### Added

- **corpus を 112 本 → 127 本にした**。足したのは 9298（2020 年代 XML）、
  6570 / 6347（2010 年代）、4949 / 4566 / 3315 / 5681 / 4034 / 3711 / 5751 /
  3411 / 2474（2000 年代）、951 / 903（1980 年代）、733（1970 年代）。
- **テストを 5 つ足した**（520 → 525）

### Known

- RFC 733（1977 年）を `A5` `A6` `E7` に、RFC 4949 を `A6` に、RFC 5751 を
  `D1` に、基準として記録した。
  - RFC 733 の題名は中央寄せの 2 行に分かれ、間に空行が入る。参考文献は
    目印を持たず、著者名から始まる段落が並ぶ。節番号はローマ数字である。
    1977 年の体裁で、ほかに同じものが無い。
  - RFC 4949 の目次は `8. Acknowledgments` だが、本文の見出しは
    `7. Acknowledgments` である。原文の側の食い違い。
  - RFC 5751 §1.6 は `Section 2.5.2.1: Deleted entire section` と書く。
    その節は削除されており、実在しないのが正しい。

## [0.6.48] - 2026-09-04

**`al.` で終わる語をすべて略語と読んでいた。** corpus を 97 本から 112 本に
広げ、RFC 5661（NFSv4.1）が出した 1 件を追ったところ、要件文 **2,488 件**が
変わる不具合だった。

### Fixed

- **略語の判定が語尾だけを見ていた**:
  - `ABBREVIATIONS` の `al.` は "et al." のためのものだが、`tail.endsWith` で
    見ていたため **`general.` `optional.` `normal.` `final.` `additional.`
    `internal.`** などがすべて略語と読まれ、そこで文が切れなかった。
  - RFC 5661 §9.1.2 の
    `(See Section 8.2.3 for a description of 'special' stateids in general.)
    Regardless of whether a stateid … the server MUST refuse service.` は、
    要件文が**注記の括弧から始まって**いた。RFC 1122 §4.2.2.7 の
    `… the TCP checksum is never optional. The sender MUST generate it …` も
    同じで、前の文がくっついていた。
  - 略語は語の頭から始まっていることを求める（直前が英字でないこと）。
  - 実測（RFC 112 本）: 要件文が変わったもの **2,488 件**、要件が 1 件増えた
    RFC が 8 本（1122 / 1123 / 2068 / 2616 / 3261 / 3550 / 5661 / 9051）。

- **1 段あたりの字下げ幅を 3 桁に決め打ちしていた**:
  - RFC 1122 は 3 桁、RFC 2328 は 4 桁で刻む。3 桁を基準に前後 2 桁で見ていた
    ため、RFC 2328 の `            12.4.1.1.  Describing point-to-point
    interfaces`（12 桁・深さ 4）が落ちていた。
  - 「段の数で割り切れて、1 段 2〜4 桁」であることを求める。付録の下位節にも
    同じ規則を当てる（RFC 2328 は `    C.1`（4 桁）と `        D.4.1`（8 桁）を
    使う）。
  - 実測: RFC 2328 の節が **120 → 151**（31 節が構造に入った）。

- **参考文献の欄が付録で閉じなかった（大文字 1 文字の見出し）**:
  - RFC 2328 は `A. OSPF data formats` と書く。`Appendix` の語だけを見ていた
    ため欄が閉じず、**付録の本文が最後の参照項目に丸ごと入って**いた。
  - 大文字 1 文字＋ピリオドまたはコロンの形も付録の始まりとする。ただし
    **読点を含まない短い題名**であることを求める。`B. Braden, R.,
    "Requirements for Internet Hosts", …` のように著者の頭文字が 1 桁目に来る
    RFC があり（RFC 1812）、これを付録と読むと参照が 0 件になる。

- **目印だけを 1 行に置く参考文献の 3 つ目の形**:
  - RFC 1812 は `   APPL:1.` と書き、引用を次の行から書く。角括弧と番号の形しか
    見ていなかったため、参照が 0 件だった。実測: RFC 1812 で **85 件**。

- **`Section N of that document` をこの RFC の節として拾っていた**:
  - RFC 8441 §4 の
    `interpreted according to Section 8.1.2.3 of [RFC7540] instead of
    Section 8.3 of that document.` は §8.3 を返していた。RFC 8441 に §8.3 は
    無い。どの文書かはここでは決められないので、参照として記録せずに落とす。

### Added

- **corpus を 97 本 → 112 本にした**。足したのは 9651 / 9530（2020 年代 XML）、
  8441 / 7541 / 7636 / 6698 / 8032（2010 年代）、5661 / 2865 / 6960 / 5730 /
  4291（2000 年代）、2328（1990 年代後半）、826（1980 年代）、1350（1990 年代
  前半）。
- **テストを 3 つ足した**（517 → 520）

### Known

- `A7` に RFC 826 を 1 件、`E5` に RFC 7541 を 1 件、基準として記録した。
  - RFC 826（ARP）は見出しを 1 つも持たない。RFC 896 と同じ形で、既に基準に
    入っている。
  - RFC 7541 の `[CRIME]` は題名が `CRIME` で、目印と同じ文字列である。
    原文がそう書いている。

## [0.6.47] - 2026-09-04

**corpus を 82 本から 97 本に広げ、9 件直した。** 足した 15 本のうち、
RFC 1812（ルータ要件）・RFC 2578（SMIv2）・RFC 5545（iCalendar）・
RFC 3164（syslog）・RFC 20（ASCII, 1969）が、それぞれ違う壊れ方を出した。

### Fixed

- **全部大文字の付録見出しを拾っていなかった**:
  - `Appendix` だけを見ていた。RFC 1812 は `APPENDIX D. Multicast Routing
    Protocols`、RFC 791 は `APPENDIX A:  Examples & Scenarios` と書く。
    区切りもピリオドだけを見ており、コロンは通らなかった。
  - 付録の始まりと分からないと、下位節（`D.1` `D.2` …）は親の文字が取れず
    まとめて落ちる。実測: RFC 1812 の **25 節**、RFC 791 の 2 節が構造に
    入った（節 194 → 219 / 14 → 16）。
  - **目次の行を付録の見出しにしない**規則も足した。番号付きの節では
    `isValidSectionHeader` が弾いているが、付録の経路には無かった。RFC 791 の
    目次 `APPENDIX A:  Examples & Scenarios ......... 34` を拾うと、本物の
    見出しが番号の重複で落ちる。

- **ABNF の注釈から要件の文を取るとき、何番目のキーワードかを数えていなかった**:
  - 組み直した文の**最初の**出現を採っていた。RFC 5545 §3.6.2 は
    ```
    ...
    ```
    と書く。`MUST` を探すと `MUST NOT` の中の `MUST` に当たり、要件の文が
    **1 つ前の文**になっていた。
  - 元の並びで何番目かを数えてから、組み直した文で同じ番号の出現を採る。
    `;` を外して空白を畳む操作はキーワードの数を変えない。
  - 実測（RFC 97 本）: 要件文が変わったもの **254 件**。ほとんどが RFC 5545 の
    ABNF 注釈で、1 つずつずれていた。

- **語を並べただけの塊から要件を出していた**:
  - RFC 2578 §3.7 は ASN.1 の予約語 96 個を 10 行にわたって並べる。その中の
    `OPTIONAL` から要件が立ち、チェックリストの 1 項目が **903 文字の語の
    並び**になっていた。
  - 小文字だけの語が 1 つも無いことで見分ける。地の文は `the` `of` `to` `is` の
    ような語を必ず含む。予約語の一覧は `BY` `FROM` `OF` `WITH` のように大文字で
    書かれるので、これらは小文字の語として数えない。
  - 実測（RFC 97 本）: **118 件**。内訳は RFC 6120 の適合性の表 52 件、
    RFC 5280 の ASN.1 のフィールド行 36 件、RFC 2131 の表の行 12 件、
    RFC 1866 の SGML DTD 6 件、RFC 5652 4 件、RFC 4120 2 件、
    RFC 821 の枠付き注意書き・RFC 2578 の予約語一覧・RFC 4253・RFC 6376・
    RFC 7489・RFC 8017 が各 1 件。
  - **これは保留になっていた設計判断（ASN.1 のフィールド行から `OPTIONAL` の
    要件を立てるか）に、測って答えを出したことになる。** `keyIdentifier [0]
    KeyIdentifier OPTIONAL,` は文ではないので落とす。

- **角括弧を使わない参考文献を拾っていなかった**:
  - RFC 3164 は `   1  Postel, J., "User Datagram Protocol", …` と番号だけを
    書く。角括弧だけを見ていたため、参考文献の欄はあるのに参照が 0 件だった
    （`E7`）。番号のあとに空白 2 個以上と大文字を求める形で拾う。
  - 実測: RFC 3164 で 9 件。

- **日付まで書くヘッダから公開年月を取れていなかった**:
  - `Month YYYY` だけを見ていた。RFC 20 は `October 16, 1969` と書く。

- **`get_related_sections` が自分自身を返していた**:
  - RFC 8017 §1 は `This document is organized as follows: Section 1 is an
    introduction …` と自分を挙げる。§1 を引くと §1 が返っていた。
    突き合わせの `X13` が見つけた。

### Added

- **corpus を 82 本 → 97 本にした**。足したのは 9250 / 9700（2020 年代 XML）、
  7950 / 6120 / 5545 / 8017 / 5905（2010 年代）、4960 / 4880 / 4287 / 3164
  （2000 年代）、1812 / 2578（1990 年代後半）、20 / 1（1960〜70 年代。新しい
  世代の枠）。
- **テストを 5 つ足した**（512 → 517）

### Known

- `A6` に RFC 2578 を 5 件、`A8` に RFC 5545 を 4 件、基準として記録した。
  - RFC 2578 の目次は §2.1〜2.6 を挙げるが、本文にその見出しは無い（§2 の
    直後は ASN.1 のモジュール定義）。原文の側の食い違いである。
  - RFC 5545 の `Sent By` `Time Zone Offset From` `Time Zone Offset To`
    `Related To` は、前置詞で終わる本物の節名である（RFC 2445 の `Sent By` と
    同じ形で、既に基準に入っている）。

## [0.6.46] - 2026-09-04

**監査が v0.6.41 から動いていなかった。** corpus を 67 本から 82 本に広げたら、
82 本すべてが例外で落ちていることが分かった。原因は v0.6.41 で足した条件
`B20` で、`context.sections`（節の**数**）を配列として扱っていた。監査は
1 件も条件を当てていなかった。

### Fixed

- **`B20` が監査全体を止めていた**:
  - `auditOne` は例外を RFC ごとに拾って先へ進む。落ちた RFC は冒頭に出るが、
    最後には「新しい破れは無し」とだけ出るので、**末尾しか見ていないと監査が
    動いていないことに気づけない**。
  - `B20` を `parsed.sections` を辿る形に直した。
  - 落ちた RFC を末尾にも出す。1 本でも落ちていれば「当てられた RFC には
    新しい破れは無し」と書き分ける。
  - v0.6.41 から v0.6.45 までの 5 版は、監査を通さずに出していたことになる。
    直したうえで 82 本に当て直し、下記を見つけた。

- **1 桁目に置かれたページフッタを落としていなかった**:
  - フッタは行末の `[Page 29]` だけを見ていた。RFC 821 は
    `[Page 68]                                                         Postel`
    と 1 桁目に置く。フッタが本文として残り、`[Page 68]` が参考文献の項目に
    なっていた（`E5`）。

- **表紙が高い RFC で公開年月を取れていなかった**:
  - 30 行までしか見ていなかった。RFC 821 は日付が 44 行目にある。60 行まで
    見る。最初に当たったものを返すので、既に取れている RFC の値は変わらない
    （実測: 82 本のうち変わったのは RFC 821 だけ）。

- **正誤表の参照に RFC 番号を付けていた**:
  - `[Err3607] RFC Errata, Erratum ID 3607, RFC 4627, <…>` から 4627 を拾い、
    `get_rfc_dependencies` が「RFC 8259 は RFC 4627 に依存する」と言っていた。
    RFC 8259 は 4627 を廃止した側である。RFC 7159 は `[Err3607]` に 3607、
    `[Err607]` に 607 という、正誤表の番号そのものを付けていた。
  - 実測（RFC 82 本）: 12 件（7159 が 2 件、8259 が 6 件、8200 が 4 件）。
    RFCXML 経路は元から番号を持たない。

- **見出しと本文が 1 行に入る形を、空白 4 個以上でしか切っていなかった**:
  - RFC 2119 は `1. MUST   This word, or the terms "REQUIRED" or "SHALL", mean
    that the` と空白 3 個で書く。節の題名が本文を丸ごと抱え込んでいた。
  - 実測（RFC 82 本）: 10 件（RFC 2119 の 5 節と、RFC 1305 の 5 節。後者は
    題名の末尾にページ番号が付いていた）。

- **名詞として置かれたキーワードから要件を出していた**:
  - BCP 14 のキーワードは名詞にもなる。RFC 2119 §7 の
    `The effects on security of not implementing a MUST or SHOULD, …` から
    4 件、RFC 5246 §1.2 の
    `Support for the SSLv2 backward-compatible hello is now a MAY, not a
    SHOULD, with sending it a SHOULD NOT.` から 3 件、RFC 9051 の付録 E の
    `(Changed from a SHOULD to a MUST.)` から 2 件が立っていた。
  - **冠詞だけを見ると採りすぎる。** 冠詞の有無だけで落とすと 36 件が消えるが、
    その中には形容詞として名詞に付く書き方が入る。`an OPTIONAL feature of
    HTTP`（RFC 9110 §14）、`the RECOMMENDED default values for these two
    parameters are 3.5 seconds …`（RFC 3550 §6.2）、`A RECOMMENDED mechanism
    to achieve this is …`（RFC 3261 §16.7）は、その節が何を選ぶべきかを
    述べている。
  - 冠詞が直前にあり、**かつキーワードの後ろに名詞が来ない**ものだけを落とす。
    実測: **16 件**（要件 11,093 件 → 11,077 件）。
  - RFC 2119 §7 の `or SHOULD` と `says MUST NOT` は残る。並列と伝聞の形は
    見分けられない。この 1 文のために仕掛けを増やさない。

- **`[RFC2045], Section 6.7:` を別文書の節として扱っていなかった**:
  - 番号のあとに来てよい文字にコロンを入れていなかった。RFC 6376 §3.2 の
    `Quoted-Printable [RFC2045], Section 6.7: any character MAY be encoded` は
    「この RFC の §6.7」になっていた（RFC 6376 に §6.7 は無い）。

### Added

- **corpus を 67 本 → 82 本にした**。足したのは
  9562 / 9421（2020 年代 XML）、8259 / 8174 / 8200 / 7252 / 8555 / 6376
  （2010 年代）、5234 / 3629 / 4120（2000 年代）、2119 / 1939（1990 年代後半）、
  959 / 821（1980 年代）。上記の不具合はすべてこの 15 本が出した。
- **監査の条件を 1 つ足した**（50 → 51）:
  `B21` キーワードが名詞として置かれた文から要件を出さない
- **テストを 8 つ足した**（504 → 512）

### Known

- `D1`（相互参照の参照先が実在する）に 2 本を基準として記録した。どちらも
  原文の側の食い違いか、機械では読めない書き方である。
  - RFC 821 §4.2 は `Section 5.3 on Sequencing and Section 5.4 State Diagrams.`
    と書くが、RFC 821 に §5.3 / §5.4 は無い（実際は §4.3 / §4.4）。
  - RFC 6376 §3.2 は `"qp-section" (ibid, Section 6.7)` と書く。`ibid` は
    直前の `[RFC2045]` を指す。82 本で `ibid` はこの 1 箇所だけなので、
    規則を足さない。
